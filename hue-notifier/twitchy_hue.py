...
class sub_alerter:
        ...
        if os.path.isfile("config.txt"):
            try:
                ...
                for setting in required_settings:
                    ...
                    # Unicode not allowed!
                    if isinstance(config[setting], str):
    ...
    def timeout(self):
        print("Timeout, allowed to flash again!")
        self.timerActive = False
    ...
    def handle_subscriber(self):
        if self.timerActive:
            print("Prohibit flash, timer is active!")
        else:
            t = Timer(20, self.timeout)
            t.start()
    ...
    def handleIRCMessage(self, ircMessage):
        ...
        if ircMessage.find("JOIN %s" %self.config['twitch_channel']) != -1:
            logger.info("Joined channel %s successfully... watching for new subscribers and messages" %self.config['twitch_channel'])
        ...
        elif ircMessage.find('PRIVMSG #') != -1:
